...
def onMouse(event, x, y, flags, param): # 마우스 이벤트 콜백 함수 구현
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        pts_cnt += 1
        if pts_cnt == 4: # 좌표가 4개 수집되면,
            # 좌표 4개 중 상하좌우 찾기
            sm = pts.sum(axis=1) # 4쌍의 좌표 각각 x+y 계산
            diff = np.diff(pts, axis=1) # 4쌍의 좌표 각각 x-y 계산

            topLeft = pts[np.argmin(sm)] # x+y가 가장 작은 값이 좌 상단 좌표
            bottomRight = pts[np.argmax(sm)] # x+y가 가장 큰 값이 우 하단 좌표
            topRight = pts[np.argmin(diff)] # x-y가 가장 작은 값이 우 상단 좌표
...
cv2.setMouseCallback(win_name, onMouse) # 마우스 콜백 함수를 GUI 윈도에 등록
cv2.waitKey(0)
